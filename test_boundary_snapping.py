...
def test_snap_mask_boundary_actually_moves():
    ...
    # Create mask with vertical boundary at x=200
    mask_256 = np.zeros((256, 256), dtype=np.uint8)
    mask_256[:, 0:100] = 1  # Left half is clothing
...
def test_snap_mask_inner_vs_outer_boundaries():
    ...
    mask_256 = np.zeros((256, 256), dtype=np.uint8)
    mask_256[100:120, 100:120] = 1  # Small 20x20 square
    ...
    original_upscaled = cv2.resize(mask_256, (512, 512), interpolation=cv2.INTER_NEAREST)
